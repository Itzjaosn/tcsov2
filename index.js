...
CACHE_REDIS=true
CACHE_LOGGING=true
CACHE_PREFIX=handler
CACHE_TTL=60
CACHE_MAX_ITEMS=5000
CACHE_CLEAN_MS=30000
...
const ConfigTemplate = {
  ...
  REDIS_URL: "string",
  CACHE_REDIS: "boolean",
  CACHE_LOGGING: "boolean",
  CACHE_PREFIX: "string",
  CACHE_TTL: "number",
  CACHE_MAX_ITEMS: "number",
  CACHE_CLEAN_MS: "number",
...
client.logs = Log;
client.cache = new Cache(config, Log);
client.cooldowns = client.cache.ns("cooldowns");
...
for (const [path, cache] of Object.entries(COMPONENT_FOLDERS)) {
  ...
  if (!existsSync(fullPath)) {
    Log.warn(
      `The '${
        path.split("/")[1]
      }' folder was not found. If you are not using one, this can be safely ignored.`
    );
...
async function HotReload(cache, componentFolder, filePath, type = 0) {
  ...
  if (isEvent) {
    ResetEvents(componentFolder);
    for (const [folder, componentCache] of Object.entries(COMPONENT_FOLDERS)) {
      if (componentCache === null) continue;
      try {
        componentCache.clear();
        ComponentLoader(folder, componentCache);
      } catch {}
    }
    return;
  }
...
(async () => {
  try {
    ...
    Log.error(`[INIT] failed to initialize: ${e?.message || e}`);
  }
