...
const { PREFIX, FANCY_ERRORS } = require("../config.js");
...
module.exports = {
  ...
  execute: async function (client, message) {
    ...
    try {
      if (command.cooldown)
        await CheckCooldown(client, message.author.id, name, command.cooldown);
      if (command.guilds) IDAccess(command.guilds, message.guildId, "Guild");
      if (command.channels)
        IDAccess(command.channels, message.channelId, "Channel");
      if (command.users) IDAccess(command.users, message.author.id, "User");
      if (command.owner)
        GuildOwner(
          message.guild ? message.guild.ownerId : undefined,
          message.author.id
        );
          ...
          message.guild === null
            ...
            : message.guild.members.cache.get(client.user.id) ||
              (await message.guild.members
                ...
                .catch(() => null));
        if (botMember !== null) {
          Permission(client, command.botPerms, botMember);
          Permission(client, command.userPerms, message.member);
        }
      }
    } catch (error) {
      const payload = {
